...
pub async fn check_token(token: &str) -> Result<(String, String, String), Box<dyn std::error::Error>> {
    let client = reqwest::Client::new();
    
    // Première requête pour obtenir l'ID d'utilisateur
    let response = client
    ...
    // Seconde requête pour obtenir les informations de l'utilisateur
    ...
    let user_info = serde_json::from_str::<serde_json::Value>(&user_response.text().await?)?;
    let username = user_info.get("username")
        .map(|s| s.as_str().unwrap_or_default())
        .unwrap_or_default()
        .to_string();

    Ok((status, user_id, username))
}
